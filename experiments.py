...
class Experiment2(Experiment):
    ...
    def run(self,
        ...
        if recurrent:
            ...
            y_ = multilayer_perceptron(x, num_input=1, num_output=2, num_nodes=n_hidden,bias=True,activation=tf.nn.relu)

        
        n_params = np.sum([np.product([xi.value for xi in x.get_shape()]) for x in tf.global_variables()])        
        #show_graph(tf.get_default_graph().as_graph_def())
        loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y,logits=y_))
