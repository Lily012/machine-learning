...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 15:04:47 2018
...
def multilayer_perceptron(x, num_nodes, num_input=1, num_output=1, activation=tf.nn.sigmoid, bias=True,
                          initializer=tf.contrib.layers.xavier_initializer(), return_weight_tensors=False):
    n_prev = num_input
    out = x
    num_layer = 0
    weights = list()
...
def recurrent_multilayer_perceptron(x, num_nodes, num_input=1, num_output=1, activation=tf.nn.sigmoid):
    ...
    assert all(x == num_nodes[0] for x in
               num_nodes)  # for a recurrent multilayer perceptron, the number of neurons in each hidden layer should be the same
...
class Dataset():
    ...
    @classmethod
    def generate_mixture_of_gaussians(cls, n, d, class_seps=[1], covariance_scale=1, test_size=0.2, one_hot=False,
                                      randomly_labeled=False, class_ratio=1, return_covariance=False, cov=None,
                                      resample=False, normalize_x=False):
